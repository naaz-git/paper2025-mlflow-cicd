...
class ModelTraining:
    ...
    def load_and_split_data(self):
        try:
            ...
            X_train = train_df.drop(columns=["patient.noshow"])
            y_train = train_df["patient.noshow"]

            X_test = test_df.drop(columns=["patient.noshow"])
            y_test = test_df["patient.noshow"]
    ...
    def evaluate_model(self , model , X_test , y_test):
        try:
            ...
            logger.info(f"Precision Score class 0: {precision[0]}")
            logger.info(f"Recall Score class 0: {recall[0]}")
            logger.info(f"Precision Score class 1: {precision[1]}")
            logger.info(f"Recall Score class 1: {recall[1]}")
            logger.info(f"F1 Score : {f1}")

            return {
                "accuracy" : accuracy,
                "precison Class 0" : precision[0],
                "precison Class 1" : precision[1],
